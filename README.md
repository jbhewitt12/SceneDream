...
If you're an author, SceneDream makes it easy to turn your own manuscript into concept art, promo images, and visual inspiration while staying in control. There is no hosted SceneDream service: the app runs locally on your machine, and the generated files are saved to your own computer.

All you'll need to get it working is an OpenAI API key. You don't need to be technically savvy to use it, because it's all done through a web interface.

You can see more examples of images generated by SceneDream on my [Flickr account](https://www.flickr.com/photos/204154573@N07/).
...
- An API key alone is not enough if your OpenAI account has no active billing or available prepaid credits. First-run extraction will now fail fast and the Documents dashboard will show a remediation hint when billing, quota, or model-access setup is wrong.
