...
class SceneExtractor:
    ...
    def extract_preview(
        ...
        *,
        ...
        for idx, chapter in enumerate(selected, start=1):
            ...
            chunks_considered = (
                min(total_chunks, chunk_limit) if chunk_limit else total_chunks
            )
        ...
        *,
        ...
        for chunk in chunks:
            ...
            if chunk.index in existing_chunk_indexes:
                ...
                print(f"  Chunk {chunk.index}: skipped (existing records found)")
                continue
            print(
    ...
    def _parse_gemini_response(
        ...
        if not isinstance(scenes_data, list):
            logger.warning(
                "Gemini returned unexpected payload for chapter %s chunk %s",
                chapter.number,
                chunk.index,
            )
            return []
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    preview.add_argument(
        "--chunks", type=int, default=1, help="Maximum chunks per chapter to process."
    )
    ...
    refine_pending.add_argument(
        "--chapter", type=int, help="Optional chapter number filter."
    )
    refine_pending.add_argument(
    ...
    with Session(engine) as session:
        ...
        for (book_slug, chapter_number), chapter_records in grouped.items():
            ...
            for batch in batches:
                try:
                    batch_refinements = refiner.refine(
                        chapter, batch, fail_on_error=True
                    )
